...
function consultarAsignacionPorId($id)
{
    ...
    try {
        ...
        $sql = "SELECT a.id, a.id_ente, a.monto_total, a.id_ejercicio, a.fecha, a.status, e.ente_nombre, e.tipo_ente 
        ...
        if ($result->num_rows > 0) {
            ...
            // Consulta adicional para obtener la distribucion asociada al id_asignacion
            $sqlDistribucion = "SELECT id, distribucion, monto_total, status, id_ejercicio 
            ...
            if ($resultDistribucion->num_rows > 0) {
                ...
                // Decodificar el campo 'distribucion' de JSON y cambiar su propiedad a 'distribucion_partidas'
                if (!empty($distribucion['distribucion'])) {
                    $distribucion['distribucion_partidas'] = json_decode($distribucion['distribucion'], true);

                    // Obtener detalles adicionales de distribucion_presupuestarias para el campo distribucion_partidas
                    ...
                    if ($resultDistribucionDetalles->num_rows > 0) {
                        ...
                        $distribucion['id_partida'] = $distribucionDetalles['id_partida'];
                        $distribucion['id_sector'] = $distribucionDetalles['id_sector'];
                        ...
                        if ($resultSector->num_rows > 0) {
                            $distribucion['sector_informacion'] = $resultSector->fetch_assoc();
                        } else {
                            $distribucion['sector_informacion'] = null;
                        }
                    } else {
                        $distribucion['id_partida'] = null;
                        $distribucion['id_sector'] = null;
                    }
                } else {
                    $distribucion['distribucion_partidas'] = [];
                }

                // Asignar la distribucion directamente al objeto asignacion
                $asignacion['distribucion'] = $distribucion;
            } else {
                $asignacion['distribucion'] = null;
            }
