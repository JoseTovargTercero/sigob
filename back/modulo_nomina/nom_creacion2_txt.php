...
if (isset($data['Venezuela']) && isset($data['Tesoro']) && isset($data['Bicentenario']) && isset($data['Caroni'])) {
   ...
   if (strlen($total) == 1) {
    ...
    fwrite($file, $header);
      ...
      if (strlen($totalcobra) == 1) {
       ...
       $line =  "0". $nrocuenta."". $totalcobra . $tipocuenta . "770" . $prueba." "."00". $cedula."003291" . "\n";
        ...
        header('Content-Disposition: attachment; filename="venezuela.txt"');
    ...
    if ($file === false) {
        ...
        // Forzar la descarga del archivo
        header('Content-Type: text/plain');
        header('Content-Disposition: attachment; filename="tesoro.txt"');
        header('Content-Length: ' . filesize($file_path));
        readfile($file_path);
    ...
    if ($file === false) {
      ...
      if (strlen($totalcobra) == 1) {
       ...
       $line =  $nacionalidad. "0". $cedula. $prueba. $nrocuenta. "". $totalcobra . "\n";
        ...
        // Forzar la descarga del archivo
        header('Content-Type: text/plain');
        header('Content-Disposition: attachment; filename="caroni.txt"');
        header('Content-Length: ' . filesize($file_path));
        readfile($file_path);
