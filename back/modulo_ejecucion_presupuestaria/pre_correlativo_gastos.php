...
$stmt = mysqli_prepare($conexion, "SELECT C.numero_compromiso 
        FROM gastos 
        INNER JOIN compromisos AS C 
            ON C.id_registro = gastos.id 
            AND C.tabla_registro = 'gastos' 
        WHERE gastos.id_ejercicio = ? 
            AND gastos.id_tipo = ?
        ORDER BY C.numero_compromiso DESC
    ");
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (strpos($numero, '-') !== false) {
            ...
            if (isset($partes[1]) && is_numeric($partes[1])) {
                $num = (int) $partes[1];
                $numeros_no_disponibles[] = $num;

                if ($num > $ultimo_numero_compromiso) {
                    $ultimo_numero_compromiso = $num;
                }
            }
        }
    }
...
$numero_recomendado = [
    ...
    'no_disponibles' => $numeros_no_disponibles
];
