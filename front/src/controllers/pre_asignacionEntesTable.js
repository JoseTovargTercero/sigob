...
import { getPreAsignacionesEntes } from '../api/pre_entes.js'
...
export const preValidateAsignacionEntesTable = async (id_ejercicio) => {
  asignacionEntesTable = new DataTable('#pre-asignacion-entes-table', {
    columns: [
      {
        data: 'ente_nombre',
...
export const loadAsignacionEntesTable = async (id_ejercicio) => {
  ...
  let asignaciones = await getPreAsignacionesEntes()
      ...
      return {
        ...
        ${
          Number(el.status) === 0
            ...
            : `
            <a
              href='pre_distribucion_detalles.php?id=${el.id}'
